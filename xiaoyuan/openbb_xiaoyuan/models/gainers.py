"""XiaoYuan Asset Performance Gainers Model."""
...
class XiaoYuanGainersQueryParams(EquityPerformanceQueryParams):
    """XiaoYuan Asset Performance Gainers Query.

    Source: https://finance.yahoo.com/screener/predefined/day_gainers
    """


class XiaoYuanGainersData(EquityPerformanceData):
    """XiaoYuan Asset Performance Gainers Data."""
...
class XiaoYuanGainersFetcher(
    Fetcher[XiaoYuanGainersQueryParams, List[XiaoYuanGainersData]]
):
    """Transform the query, extract and transform the data from the XiaoYuan endpoints."""

    @staticmethod
    def transform_query(params: Dict[str, Any]) -> XiaoYuanGainersQueryParams:
        """Transform query params."""
        return XiaoYuanGainersQueryParams(**params)

    @staticmethod
    def extract_data(
        query: XiaoYuanGainersQueryParams,
        ...
        **kwargs: Any,
        ...
        factors = ["收盘价（不复权）", "成交量（不复权）", "总市值", "市盈率（滚动）"]
        sc = f"""
            use mytt
            ...
            t = select * from t where percent_change > 0.03 and 收盘价（不复权） >= 5.0 and 成交量（不复权） > 15000 and 总市值 > 1000000000
    ...
    def transform_data(
        ...
        **kwargs: Any,
        ...
        if query.sort == "desc":
            ...
            ascending = True
        return [
            XiaoYuanGainersData.model_validate(d)
